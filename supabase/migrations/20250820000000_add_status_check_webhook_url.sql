...
COMMENT ON COLUMN public.courier_webhook_settings.status_check_webhook_url IS 'Separate webhook URL for checking courier status updates (GET requests to Pathao API)';

-- Update existing records to set a default status check URL
UPDATE public.courier_webhook_settings
